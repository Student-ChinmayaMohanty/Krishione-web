...
// Mock: Pretend we call a crop disease model. Returns a diagnosis from filename/content hash.
async function mockDetectDisease(file) {
  await wait(900);
  const names = [
    {
      name: "Leaf Blight",
      remedy:
        "Use Mancozeb 75% WP, improve field drainage, remove infected leaves.",
    },
    {
      name: "Powdery Mildew",
      remedy:
        "Apply Sulfur-based fungicide in early morning, avoid overhead irrigation.",
    },
    {
      name: "Rust",
      remedy:
        "Use Triazole fungicide; rotate crops; resistant varieties preferred.",
    },
    {
      name: "Healthy",
      remedy:
        "No visible disease. Maintain balanced NPK and regular scouting.",
    },
    {
      name: "Bacterial Spot",
      remedy:
        "Copper oxychloride spray; sanitize tools; avoid leaf wetness.",
    },
  ];
  // pseudo-random by file size
  const idx = file?.size ? file.size % names.length : Math.floor(Math.random() * names.length);
  return names[idx];
}
...
function CropDoctorPage() {
  ...
  const onFile = async (f) => {
    ...
    setLoading(true);
    const res = await mockDetectDisease(f);
    setResult(res);
    setLoading(false);
  };
